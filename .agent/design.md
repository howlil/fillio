...
MVP stores document metadata and preferred-document references only. Do not store large PDF binaries in `chrome.storage.local` as part of the first vertical slice.
...
Browser-level tests cover only behavior that needs the DOM/extension runtime: extraction, content/background messaging, dynamic rescan, real fill event behavior, floating UI isolation, and critical vault flow.
